"""
Power Spectra - most of the code from ``agpy`` of Adam Ginsburg
"""
...
def PSD2(image, image2=None, oned=False,
    ...
    if wavnum_scale:
        wx = numpy.concatenate([numpy.arange(image.shape[0] / 2, dtype='float'),
                                image.shape[0] / 2 - numpy.arange(image.shape[0] / 2, dtype='float') - 1]) / (
                         image.shape[0] / 2.)
        wy = numpy.concatenate([numpy.arange(image.shape[1] / 2, dtype='float'),
                                image.shape[1] / 2 - numpy.arange(image.shape[1] / 2, dtype='float') - 1]) / (
                         image.shape[1] / 2.)
        wx /= wx.max()
        wy /= wy.max()
...
if __name__ == "__main__":
    # Path to FITS file with image
    fits_image = "/home/ilya/data/deep_clean/cc.fits"
    from astropy.io.fits import getdata
    image = getdata(fits_image)
    image = image.squeeze()

    # Here ``image`` msut be 2D numpy array
    psd2 = PSD2(image)
    spec = pspec(psd2, view=True)